  ...
  <div class="flight-item">
    <div @click="isShow = !isShow">
      ...
      <el-row type="flex" align="middle" class="flight-info">
        <el-col :span="6">
          <span>{{ data.airline_name }} </span> {{ data.flight_no }}</el-col
        >
        <el-col :span="12">
          <el-row
            type="flex"
...
export default {
  ...
  computed: {
    intervalTime() {
      //起飞时间
      const dep = this.data.dep_time; //20:50
      //到达时间
      const arr = this.data.arr_time; //22:50
      // 分割出 时 分
      const start = dep.split(":"); //[20,50]
      const end = arr.split(":"); //[22,50]
      //隔天到达
      if (end[0] < start[0]) {
        end[0] = +end[0] + 24;
      }
